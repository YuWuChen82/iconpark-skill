...
`assets/goodcase/` 命名格式 `中文名_英文名.svg`，host agent 推荐命名时直接 `ls` 该目录学习风格。主分类分布（31 个反推）：**界面组件**(12)·**硬件**(4)·编辑(2)·表情(2)·交流沟通(2)·链接(2)·多媒体音乐(2)·用户人名(2)·天气(1)·时间日期(1)·办公文档(1)。

> 规律：中文名取"形状/功能"，英文名取"行业通用名"(kebab-case)，同义多图标用 `-one`/`-two` 区分。

### badcase（13 个问题案例，4 类典型问题）

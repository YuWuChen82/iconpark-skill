...
> - **路径选择是 host agent 内部思考,不要在输出中告诉设计师** — 详见下面 §🔇 内部思考与外部输出分离

## 🔇 内部思考与外部输出分离(设计师只看结果,不看过程)

> **核心原则**:host agent 的**内部动作**和**外部输出**必须严格分离 —— 设计师只看到**结果**(弹出的选项 / 预分析卡片 / 最终卡片 / 错误信息),**看不到** host agent 的思考过程(检测 runtime、选哪条路径、为什么选)。

### 内部动作(不输出)

以下动作**必须在 host agent 内部完成**,**禁止**作为文本输出给设计师:

- ❌ "原生 `request_user_input` 在 Default 模式不可用,按 §路径 B 降级协议执行"
- ❌ "我跑在 Codex CLI,所以用 `request_user_input` 工具"
- ❌ "Claude Code 没有 `request_user_input`,只能用 `AskUserQuestion`"
- ❌ "我先按主推继续,稍后改" (host agent 内部决策过程)
- ❌ "我看到你的回应了,现在进入第 3 轮" (状态汇报,设计师能从流程节点推出来)
- ❌ "因为我跑了 dry_run 验证..." (评估/调试痕迹)
- ❌ "现在解析 SVG 内 metadata..." (步骤进度,设计师能从结果看出来)
- ❌ "置信度计算逻辑:metadata 完整度..." (内部推理)

### 外部输出(给设计师看)

以下内容**才是** host agent 应该输出的:

- ✅ 预分析卡片(节点 A.1)
- ✅ 弹出的选项(节点 A.2 / B.2 / C.1 / D.1 / E.1 / F.1 / G.2)
- ✅ 最终结果卡片(节点 G.1)
- ✅ 错误信息(读 SVG 失败 / 命名校验失败 / 弹超时终止)
- ✅ 设计师直接问的"为什么"问题(给出**结论**,不是推理过程)

### 对照表

| 场景 | ❌ 暴露内部思考(违规) | ✅ 只输出结果(正确) |
|---|---|---|
| 降级 runtime | "原生工具不可用,按 §路径 B 走纯文字" | (静默)直接给 B.1 简洁编号列表 |
| 选 AskUserQuestion | "我用 Claude Code 的 AskUserQuestion 调你" | (静默)直接弹选项 |
| 步骤间过渡 | "我先按主推继续,稍后改" | (静默)直接给下一步内容 |
| 弹完选项后 | "我看到你选了奖杯,现在进入第 3 轮辅分类" | (静默)直接给辅分类弹选项 |
| 错误处理 | "因为我检测到 host agent 调错 runtime..." | 直接给错误信息:"找不到 request_user_input 工具,请切换到 Claude Code 或启用 Codex 实验标志" |
| 置信度解释 | "置信度计算逻辑:metadata 完整度 + badcase 命中..." | 直接给结果:"置信度:medium(理由:含未识别中文)" |
| 调试信息 | "我现在解析 path d 命令..." | (静默)等结果出来 |

### 例外:设计师显式问"为什么"才解释

- 设计师: "为什么不直接用 jc-icon-star?"
- host agent: "因为 SVG 是双星重叠,不是单星。改成 jc-icon-sparkle 或 jc-icon-stars 更准"
- ✅ 这是**直接回答设计师问题**,不是"暴露内部思考"

### 强约束

> 🛑 **host agent 默认行为**:完成内部动作后**直接输出结果**,中间不夹任何"我现在 / 我看到 / 我选择 / 按 X 协议"这类元说明。
>
> 🛑 **反例黑名单**(直接套用 §🛑 STOP 红线 #5-#8):所有"host agent 内部决策过程"都按"干巴巴弹选项"处理,违反本节等于**违反强制约束 #2 + #3**。
...
- 9. **🆕 禁止暴露 host agent 内部思考** —— runtime 检测 / 路径选择 / 工具决策 / 步骤进度 / 内部推理等都是内部动作,设计师**不关心**,**禁止**作为文本输出。详见 §🔇 内部思考与外部输出分离
...
| **🆕 暴露 host agent 内部思考** | 🔴 把"原生工具不可用,按 §路径 B 降级" / "我先按主推继续" / "我看到你选了 X" 等内部动作作为文本输出 — 设计师**不关心** host agent 怎么想,只关心**结果** | 内部动作**全部静默执行**,**只输出结果**(弹选项 / 卡片 / 错误);设计师显式问"为什么"才解释 |
